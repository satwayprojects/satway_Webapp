...
from .forms import CreateSubDealerForm1, CreateUserForm1, CreateSubDealerForm2, CreateUserForm2, DeviceVehicleAllocationForm, AllocateDevice, AllocateDeviceUser
...
def dashboard(request):
    ...
    subdealercount = User.objects.filter(added_by = username,user_type = "SD").count()
    ...
    subdealers = User.objects.filter(added_by = username,user_type = "SD").values('username','phone')[:10]

    context = {"total":total_count, "sold":sold_count,"user":usercount,"subdealer" : subdealercount,"ready":readyToSell,'deactive_stock':deactive_stock,'deactive_sold':deactive_sold,"userslist" : users, "dealerlist" :subdealers}
...
def inventory(request):
    ...
    if(request.GET.get('search')):
        device_object = deviceTable.objects.filter(imei__contains = request.GET.get('search'),current_owner_id = currentUsername) | deviceTable.objects.filter(transactiontable__held_by = currentUsername, imei__contains = request.GET.get('search'))[:10]
        ...
        page_num = request.GET.get('page',1)
        device_object = deviceTable.objects.filter(current_owner_id = currentUsername).distinct() | deviceTable.objects.filter(transactiontable__held_by = currentUsername).distinct()
        stock_paginator = Paginator(device_object, 10)
        try:
            stock_page = stock_paginator.page(page_num)
        except:
...
def inventory_active(request):
    ...
    if(request.GET.get('search')):
        ...
        page_num = request.GET.get('page',1)
        device_object = deviceTable.objects.filter(activation_date__gt=renewal,current_owner_id = currentUsername).order_by('activation_date')
        stock_paginator = Paginator(device_object, 10)
        try:
            stock_page = stock_paginator.page(page_num)
        except:
...
def inventory_dealer(request):
    ...
    if(request.GET.get('term') == "P"):
        ...
        page_num = request.GET.get('page',1)
        device_object = deviceTable.objects.filter(current_owner_id = request.GET.get('dealer')).order_by('activation_date')
        stock_paginator = Paginator(device_object, 10)
        try:
            stock_page = stock_paginator.page(page_num)
        except:
...
def userRegisteration(request):
    ...
    if request.method =='POST':
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            return render(request,'dealer/add_user.html',context)
...
@login_required
def subDealerRegisteration(request):
    currentuser = request.user
    dealer = currentuser.username
    if(currentuser.user_type != 'DE'):
        # return 404
        return render(request,'dealer/error404.html')
    if request.method =='POST':
        form1 = CreateSubDealerForm1(request.POST)
        form2 = CreateSubDealerForm2(request.POST)
        # print("Here1")
        if form1.is_valid() and form2.is_valid():
            current_user = request.user  #retuens User Object [username = Amal, user_type = US]
            #print("Current User = ",current_user.username)
            subdealername = request.POST['username']
            email = request.POST['email']
            password = request.POST['password']
            firstName = request.POST['first_name']
            phone = request.POST['phone']
            ...
            User.objects.create_user(username=subdealername,
                                     first_name=firstName,
                                     email=email,
                                     password=password,
                                     user_type='SD',
                                     ...
                                     phone=phone,
                                     company_details=company)
            form1 = CreateSubDealerForm1()
            form2 = CreateSubDealerForm2()
            context = {'form1':form1,'form2':form2,'success_message': 'Sub-Dealer Registration Successful.'}             
            return render(request,'dealer/add_sub_dealer.html',context)
...
@login_required
def deviceAllocationSubDealer(request):
    currentuser=request.user
    currentusername = currentuser.username
    if(currentuser.user_type != 'DE'):
        # return 404
            ...
            try:
                for record in imeiList:
                    ...
                    try:
                        ...
                        return render(request,'dealer/allocation_sub_dealer.html',context)
                    ...
                    if(currentuser.user_type != "AD"):
                        transactionTable.objects.filter(imei = record,sold_to_id = currentusername).update(last_transaction = False)
...
def editprofile(request):
    ...
    if request.method=="GET":
        return render(request,'dealer/profile.html',{'data': profile_details})
    else:
...
@login_required
def ajax_load_subdealer(request):
    currentuser = request.user
    username = currentuser.username
    userobject = User.objects.filter(username__istartswith=request.POST['username'],added_by = username,user_type = "SD").values('username','phone')[:10]
    return JsonResponse({"imeiObject" :  list(userobject)})
